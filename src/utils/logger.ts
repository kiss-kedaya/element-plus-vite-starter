...
export class Logger {
   ...
   * 获取性能信息
   */
  private getPerformanceInfo(): { memory?: number; timing?: number } | undefined {
  ...
  private saveToStorage(entry: LogEntry): void {
    try {
      ...
      if (existingLogs.length > this.config.maxStorageSize) {
        existingLogs.splice(0, existingLogs.length - this.config.maxStorageSize)
      }
  ...
  getStats(): any {
    ...
    this.logBuffer.forEach(entry => {
      const levelName = LogLevel[entry.level].toLowerCase() as keyof typeof stats.byLevel
